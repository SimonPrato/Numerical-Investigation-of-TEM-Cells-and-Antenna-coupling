...
\begin{figure}[t]
	...
	\caption{TEM cell with an arbitrary current source $\mathbf{J}$ along the curve $\mathbf{\tau}$. $\mathbf{E}$ and $\mathbf{H}$ are the field intensities induced by the current. $\mathbf{E}^+$ and $\mathbf{E}^-$ are outgoing fields towards both output ports of the TEM cell. $\mathbf{S}$ indicates the surface, and $V$ the volume of the domain in question. \colorbox{red}{Important: Only works for symmetrical waveguides. Collin did this in circular waveguides. Maybe replace tau with a dipolmoments in the center. Make a separate graphic for the TEM cell.}}
