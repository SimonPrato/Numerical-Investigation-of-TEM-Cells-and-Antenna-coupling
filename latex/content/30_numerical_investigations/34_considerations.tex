...
The investigations in this thesis focus on the coupling behavior of antennas. All conducting surfaces in the simulation models are perfect electric conductors (PEC) to remove the influence of lossy conductors, which are especially significant due to the Skin-effect. Conductor losses contribute to the power consumption of the small loop antenna, for example, and is significantly larger than radiation power \cite[p. 231]{Balanis_1997}.

The first two antennas investigated are the monopole (\cref{sec:monopole}) and the loop antenna (\cref{sec:loop_sim}), which represent the most basic antennas producing electric and magnetic dipole moments, respectively. Other electrically small antennas deliver similar results, depending on wether they are capacitive or inductive. Capacitive antennas are analogous to monopole antennas, and inductive ones to the loop antenna, both including their equivalent circuits. This applies even for electrically small antennas with high Q-factor.


\subsubsection{TEM cell model}\label{sec:tem_cell_model}

The TEM cell model used, shown in \autoref{fig:tem_cell_model2}, has a width of $a=40$\,mm, a height of $b=24$\,mm and a length of $l=100$\,mm. The cut-off frequencies of higher-order modes are demonstrated in \autoref{fig:te01_te10_tem_propagation}. In following investigations focusing on the TEM mode, the frequency range of interest spans from 1\,MHz to 3\,GHz. If the TE\textsubscript{01} mode is included in the investigations, the upper limit of the frequency range is extended to 3.3\,GHz.

The cell walls and septum are modeled as PECs. In the TEM cell simulation model, the tapered transition sections at the ports are omitted. As discussed in \cref{sec:higher-order-modes}, this simplification allows unrestricted propagation of higher-order modes, facilitating investigations of their coupling behavior with antennas. The reference impedances of the output ports equal $Z_w \approx 50\,\Omega$.
...
\begin{figure}[htbp]
	...
	\caption{Circuit representing the TEM cell, where the capacitance and inductance is split into two separate, equal components $C_T=C_{T1} + C_{T2}$, $L_T=L_{T1} + L_{T2}$ and $C_{T1} = C_{T2}$, $L_{T1} = L_{T2}$. The resistances $R_1, R_2$ at the output ports model the reference impedance $Z_w \approx 50\,\Omega$.}
...
Placing $\mathbf{m}_{\mathrm{m}}$ and $\mathbf{m}_{\mathrm{e}}$ in the center of the TEM cell therefore significantly simplifies modeling electrically small antennas with equivalent dipole moments. This assumption is valid for the TEM mode. This configuration is assumed for all numerical investigations following in this thesis, unless otherwise stated.
 ...
 When normalizing to the free-space wave impedance $\eta_0$, $\mathbf{m}_\mathrm{e}$ can be interchanged with an equivalent $\mathbf{m}_\mathrm{m}$ and vice-versa \cite[p. 414]{Jackson}. Therefore, normalizing either $\mathbf{m}_{\mathrm{e}}$ or $\mathbf{m}_{\mathrm{m}}$ to the free-space wave impedance $Z_0$ enables a meaningful comparison between them.
...
Lastly, it is best practice to select the upper limit of the investigated frequency range as the solution frequency. This choice produces the largest spatial field changes, forcing the adaptive mesh process to generate a high-resolution mesh. Results in the frequency range below the solution frequency benefit from a reusable mesh that is finer than required. This procedure is applied for every numerically derived frequency-dependent result presented in this thesis.

\subsubsection{Shielding material models}\label{sec:bc-shield}

The numerical investigation of large computational domains containing thin shielding material requires large mesh sizes for accurate representation of fields around the shield. Especially in the analysis of the ASTM ES7-83 method in \cref{sec:astm-sim}, the shielding material leads to high computational effort. A solution is the implementation of boundary conditions on the shielding material's surface, as described in \cite{1430946}.

In this solution process, the shielding material is removed from the computational domain. Instead, the boundary conditions imposed on its surface are considered, and plane wave propagation normal to the surface is assumed. 
...
%This analysis examines how dipole moments behave inside TEM cells of different sizes. The main goal is to prove that the way these antennas respond to various frequencies depends on their own shape, rather than the specific size of the testing cell. By scaling the cell geometry while keeping the electrical properties constant, we aim to show that the resulting signal patterns remain identical once they are normalized. This study helps confirm that measurements taken in a small, controlled environment are representative of the antenna's true physical characteristics across the entire frequency spectrum.
%
%This section examines the correlation between TEM cell dimensions and electromagnetic coupling to ensure that the coupling mechanisms remain invariant to shifts in the dominant field regions. 

This chapter investigates how changes in the dominant coupling field region over frequency between an antenna and a TEM cell influence the non-linear frequency behavior observed in output power measurements. To clarify this relationship, a numerical analysis was conducted by evaluating the dipole moments in both a large and a small TEM cell, as shown in \autoref{fig:krtemcell}. The resulting output powers were compared to assess the impact of varying coupling field regions on the frequency response. In particular, this analysis aims to determine whether the frequency-dependent coupling behavior arises primarily from the antenna geometry itself or from the associated coupling field regions.

%This section answers the question, if the frequency-dependent behavior of the output power produced is intrinsic to the antenna's geometry. By systematically scaling the TEM cell dimensions while maintaining a constant impedance $Z_0 \approx 50\,\Omega$, the influence of specific field regions ($k \cdot r$) is isolated. The objective is to demonstrate that the normalized output power $P_\mathrm{norm}(f)$ remains congruent across different cell sizes, thereby proving that the non-linear coupling characteristic is independent of the surrounding environment and solely defined by the antenna's equivalent dipole moments $\mathbf{m}_\mathrm{e}$ and $\mathbf{m}_\mathrm{m}$.

%An electric dipole moment $\mathbf{m}_e$ and a magnetic dipole moment $\mathbf{m}_m$ are located within the TEM cell numerical model, as shown in \autoref{fig:krtemcell}. 
...
The large TEM cell has dimensions of $a=40,\mathrm{mm}$ and $b=24,\mathrm{mm}$, while the small TEM cell has $a=10,\mathrm{mm}$ and $b=6,\mathrm{mm}$. Both configurations maintain a characteristic impedance of $Z_0 \approx 50,\Omega$. The dipole moments exhibit the behavior shown in \autoref{fig:linearmomentws}. The output power, normalized to unity for both the large and the small TEM cell, is presented in \autoref{fig:small-large-tem-power1}. The agreement between the output powers indicates that changes in the coupling field regions over frequency between the TEM cell and the source are minimal.
...
%\begin{figure}[htbp]
%	\centering
%	\includegraphics[width=1\linewidth]{content/img/small-large-tem-power}
%	\caption{Comparison of output powers between small and large TEM cell.}
%	\label{fig:small-large-tem-power}
%\end{figure}
