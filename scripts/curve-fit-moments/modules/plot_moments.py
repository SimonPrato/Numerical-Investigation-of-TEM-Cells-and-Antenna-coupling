...
def plot_moments(m_e, m_m, frequencies, antenna):
    ...
    # Titel und Anzeige
    fig.suptitle(f'Dipole moments of {antenna} antenna in TEM cell over frequency')
    fig.tight_layout()
    ...
    ax1.set_xlim(np.min(frequencies/1e9), np.max(3))
    ax2.set_xlim(np.min(frequencies/1e9), np.max(3))
