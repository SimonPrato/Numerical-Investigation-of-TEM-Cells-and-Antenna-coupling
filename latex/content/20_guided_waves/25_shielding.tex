\subsection{Shielding}\label{sec:shielding}

The fundamental mechanisms of electromagnetic shielding are reviewed, 
covering the contributions of reflection, absorption, and multiple internal 
reflections to the total shielding effectiveness. The influence of a material's 
conductivity, permittivity, and permeability on each mechanism is discussed, 
followed by a description of the ASTM ES7-83 method and the dual TEM cell 
methods used to evaluate electric and magnetic near- and far-field shielding effectiveness.
...
Reflections are caused by wave impedance mismatch between two materials. It is common practice to normalize the wave impedance $\eta_n$ by the free-space wave impedance $\eta_0$ when determining the reflection coefficient $R$. At the interface between free space and a shielding material, this yields \cite[p.~183]{Collin_2015}

\begin{equation}
	R=\frac{\eta_n-1}{\eta_n+1}.
...
\begin{equation}
\eta_n=\frac{1}{\eta_0}\sqrt{\frac{j\omega\mu}{\sigma+j\omega\epsilon}}.
...
In contrast, magnetic fields are predominant in the near-field region of magnetic dipoles, as demonstrated with \eqref{eqn:pr_loop}. A material with high conductivity shields well against high-frequency magnetic fields due to the creation of counter-acting eddy currents \cite[p.~112]{Jaroszewski_Thomas_Rane_2019}. Low-frequency magnetic fields are difficult to shield, and the most common approach is the redirection of magnetic flux lines due to high permeability of the shielding material \cite[pp.~112-113]{Jaroszewski_Thomas_Rane_2019}.
...
Multiple internal reflections may occur when electromagnetic waves reach the 
opposite boundary of the shielding material. When the material thickness 
exceeds the skin depth, the corresponding correction factor $B_{\mathrm{dB}}$ 
is negligibly small and can be omitted. When the material thickness is smaller 
than the skin depth, however, these internal reflections interfere with the 
primary reflected wave, thereby reducing the overall shielding effectiveness, 
and $B_{\mathrm{dB}}$ takes a negative value \cite[p.~1]{Jaroszewski_Thomas_Rane_2019}.
...
The ASTM ES7-83 method, an Emergency Standard issued by the American 
Society for Testing and Materials (ASTM) in 1983, is a standardized 
procedure used to determine the shielding effectiveness of planar 
materials under far-field conditions. The shielding material is inserted 
into a TEM cell around the septum, and ideally forms a continuous 
connection with the TEM cell walls \cite{MORARI_BĂLAN_2015}.
...
When applying numerical analysis in combination with this method, it is more convenient to define a reference output power $P_\mathrm{ref}$ for an unloaded TEM cell, and an output power for the loaded case $P_\mathrm{load}$. This leads to a similar expression, 
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption{Cross section of shielding material in TEM cell in the (a) $yz$-plane and in the (b) $xy$-plane.}
    \label{fig:subfigures}
\end{figure}

Then, the S-parameters derived in the simulations are used to determine the output powers $P_\mathrm{ref}$ and $P_\mathrm{load}$. By exciting the TEM cell with a power of 1\,W, the reference power is $P_\mathrm{ref}=1\,\mathrm{W}$. The measured power is then derived through
...
low-impedance fields. The shielding performance of a material depends not only on its electric properties but also on the degree of impedance mismatch between the material and the incident near-field, as expressed by \eqref{eqn:reflection_coefficient_plane_dielectric}. Consequently, $SE_\mathrm{dB}^\mathrm{m}$ characterizes the shielding against magnetic near-field sources  \cite{4091811}, for which the material's impedance mismatch with low-impedance fields is decisive, while e $SE_\mathrm{dB}^\mathrm{e}$ is the relevant metric for electric near-field sources. 
