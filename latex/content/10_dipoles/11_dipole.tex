...
\autoref{eqn:compl_power_inf_elec_dipole} demonstrates, that the imaginary part of the power radiated by the infinitesimal electric dipole shows capacitive behavior.
...
\begin{equation}
    \mathbf{m}=\frac{1}{2}\iiint_V (\mathbf{x'} \times \mathbf{J})\,dv'.
...
\subsection{Dipoles modeling Antennas}

The infinitesimal electric and magnetic dipoles discussed in \autoref{sec:ele_dip} and \autoref{sec:mag_dip} can model electrically small antennas. Using Ampère-Maxwell Law in matter

\begin{equation}
	\nabla \times \mathbf{H} = \mathbf{J}_\mathrm{f} + \frac{\partial
	 \mathbf{D}}{\partial t}
	 \label{eqn:ampere-maxwell}
\end{equation}

\begin{equation}
	\oint_C \mathbf{H} \cdot d\mathbf{s}' = I_f + \epsilon \frac{\partial \phi}{\partial t} 
\end{equation}
...
Analogous to the Ampère-Maxwell Law in \autoref{eqn:ampere-maxwell}
...
Relating them with the antenna impedance $Z = U/I$

\begin{equation}
	\frac{I_m}{I_e} = \frac{Z-j\omega L }{1-j\omega CZ}
\end{equation}
