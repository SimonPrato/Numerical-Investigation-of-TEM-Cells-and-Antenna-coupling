...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.4\linewidth]{content/img/rect_waveguide_tm}
	\caption{TM mode in rectangular waveguide}
...
\begin{figure}[t]
	...
	\caption{TEM cell with an arbitrary current source $\mathbf{J}$ along the curve $\boldsymbol{\tau}$. $\mathbf{E}$ and $\mathbf{H}$ are the field intensities induced by the current. $\mathbf{E}^+$ and $\mathbf{E}^-$ are outgoing fields towards both output ports of the TEM cell. $\mathbf{S}$ indicates the surface, and $V$ the volume of the domain in question.}
