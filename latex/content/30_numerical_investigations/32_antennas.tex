...
The current $I$ in the loop antenna reduces along the antenna wire, indicating displacement current coupling back to the septum and back to the feedpoint, also seen in the reducing magnetic field intensity in \autoref{fig:loopnearfield}. Consequently, $\mathbf{m}_e$ gains a significant magnitude according to \autoref{eqn:me_i}, influencing the electric coupling behavior of the antenna. 



\todo[inline]{insert current return plot. Insert free-space PEC loop simulations.}
...
The increases in voltage and decrease in current follows from the impedance, depicted in \autoref{fig:currimp}. The strongly inductive impedance increases linearly over frequency, and keeping the input power constant leads to the observed behavior. 
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[b]{1\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{content/img/loop_surface_currents_offset_rotation}
		\caption{Surface current density of loop antenna with offset of $x=7\,\mathrm{mm}$ and a 90$\textdegree$ rotation angle at 3\,GHz}
...
\begin{figure}[htbp]
	...
	\caption{Output power transmitted by the antenna to an output ports through the TEM and TE\textsubscript{01} separately over frequency.}
	\label{fig:loopsurfacecurrentsoffsetrotation3ghz3plot}
\end{figure}

The TE01 mode transmits power in phase. It starts transmitting at the cut-off frequency, for which the rotated loop antenna with the offset is very susceptible, because its magnetic dipole moment aligns with the magnetic fields of this mode.
