...
\begin{figure}[htbp]
	...
	\caption{Shielding effectiveness of a sheet of ferrite, barium titanate and silver over frequency determined with the ASTM ES7-83 method.}
	\label{fig:se-barrium-ferrite}
\end{figure}


A model of a TEM cell containing a sheet of a shielding material according to the ASTM ES7-83 method described in \cref{sec:astm} is used to determine the shielding effectiveness of barium titanate and ferrite, shown in \autoref{fig:se-barrium-ferrite}. The shielding material sheet has a thickness of 10\,µm, and is located in the center of the TEM cell at $z=0$. The sheet is modeled with impedance boundary conditions to significantly reduce computational effort, as discussed in \cref{sec:bc-shield}. 

A reference power of $P_\mathrm{ref}=1\,\mathrm{W}$ is chosen and the load power $P_\mathrm{load}$ is numerically derived. \autoref{eqn:SE_power} yields the shielding effectiveness $SE_\mathrm{dB}$ of the material over frequency. The properties of the materials used are listed in \autoref{tab:materials}. Ferrite additionally is modeled to have a magnetic loss tangent of $\tan \delta_m = 0.05$ and barium titanate an electric loss tangent of $\tan \delta = ?$.\todo{Loss tangent}
...
\begin{table}[h]
	...
	{\renewcommand{\arraystretch}{1.2}
		...
		\begin{tabular}{|l|c|c|c|}
			...
			\hline
			Silver & $\approx 1$ & $\approx 1$ & $6.10\cdot 10^{7}$\,S/m \\
			\hline
	\end{tabular}}
...
At low frequencies, the non-conductive materials exhibit low shielding efficiency despite having a high reflection coefficient. This reduced efficiency is primarily due to their low absorption coefficients, which allow more electromagnetic energy to penetrate and transmit through the shielding material. As frequency increases, the shielding material becomes electrically thicker, leading to greater absorption losses and, consequently, improved shielding efficiency. 

Silver, as a conductive material, exhibits a high reflection coefficient due to the significant impedance mismatch with air. This impedance mismatch decreases as frequency increases. Additionally, silver has a high absorption coefficient, which grows with increasing frequency. This trend is consistent with the reduction in skin depth at higher frequencies, as discussed in \cref{sec:skin_effect}. Within the frequency range considered, the decrease in the reflection coefficient is more pronounced than the increase in the absorption coefficient, resulting in a net decrease in shielding effectiveness with increasing frequency.

\FloatBarrier
\subsection{Dual TEM cell}

A simulation model of two TEM cells as shown in \autoref{fig:dual_tem_cell}, consisting of the models demonstrated in \cref{sec:tem_cell_model}, without the tapered sections, is created. An empty square aperture with a side length of $d=10\,\mathrm{mm}$ is used, which makes it electrically small up to a frequency of approximately $3\,\mathrm{GHz}$. Significant numerical error may result from insufficient mesh resolution in the aperture region, therefore, 10 to 15 mesh elements across the aperture are desirable to guarantee accurate results. The procedure described in \cref{sec:dual_tem_cell} is applied to derive the electric and magnetic shielding effectiveness of, again, ferrite and barium titanate (BaTiO\textsubscript{3}).
...
\begin{figure}[htbp]
	...
	\caption{The electric $SE_\mathrm{dB}^\mathrm{e}$ and magnetic $SE_\mathrm{dB}^\mathrm{m}$ shielding effectiveness derived with (\ref{eqn:se_dual_cell_e,eqn:se_dual_cell_m}).}
	\label{fig:se_e_m}
\end{figure}
...
Similarly, \autoref{fig:ferriteenclosurepower} shows the radiated power from the antennas with and without the ferrite enclosure. In this case, the output power of the monopole antenna increases due to the ferrite enclosure inductively loading the antenna. The enclosure dampens the loop antenna’s radiation, which can be attributed to the redirection of magnetic flux in the material, and the power dispersion induced by the magnetic loss tangent and conductivity.
...
\begin{figure}[htbp]
	...
	\caption{Output power over frequency produced by antennas with varying enclosure thickness.}
...
\begin{figure}[htbp]
	...
	\caption{Electric near-field distribution of the loop antenna at the resonance frequency of 2.5\,GHz, with a barium titanate shielding enclosure of 100\,µm thickness.}
