...
The mesh determines the resolution of the field quantities over the computational domain. Since electrically small conductors are involved, a high mesh resolution is required in their proximity. The maximum mesh element length on error-prone volumes are adjusted, until the obtained results do not fluctuate significantly over frequency. Such volumes are usually located adjacent to the feedpoint and on conductor edges. There, a number of roughly 15 elements across the surface of the problem volume suffice to model the region well, without exploding the mesh size.
...
The monopole antenna, shown in \autoref{fig:monopoleantenna}, is installed at the center of the TEM cell and connected to the feedpoint located on the top wall. It has a length of 5\,mm, which makes it electrically short for frequencies up to 6\,GHz. It may be approximated as an infinitesimal electric dipole for frequencies up to 1.25\,GHz. The equivalent dipole moments $\mathbf{m}_e$ and $\mathbf{m}_m$ are derived through \crefrange{eqn:ifa_me}{eqn:ifa_mm}. 
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{content/img/loop-feed-voltage-current}
		\caption{Feed voltage and current at the feedpoint of the antenna over frequency.}
		\label{fig:loop-feed-voltage-current}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{content/img/monopole_imp}
		\caption{Magnitude and phase of the antenna impedance over frequency.}
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{Electric field and power of at the output port.}
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{Comparison of output power produced by monopole antenna and equivalent dipole moments, respectively.}
		\label{fig:monopolemomentcomp}
	\end{subfigure}
	
	\caption{Investigation of the output power produced by the dipole moments and loop antenna to demonstrate validity of the model.}
