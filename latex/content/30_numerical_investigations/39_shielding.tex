...
Barium titanate exhibits good electric shielding characteristics, but demonstrates a negative $SE_\mathrm{dB}^\mathrm{m}$ value, indicating poor magnetic shielding. Negative shield effectiveness values are possible, and occur due to interference patterns at the filled aperture, which cause $a$ and the output power at port 3 to increase \cite{4091811}. Ferrite, on the other hand, demonstrates a higher magnetic shielding effectiveness $SE_\mathrm{dB}^\mathrm{m}$, but a low or negative electric shielding effectiveness $SE_\mathrm{dB}^\mathrm{e}$. 
