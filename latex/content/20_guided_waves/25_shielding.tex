...
\subsubsection{Incident, reflected and transmitted waves in medium}\label{sec:irt_waves}
...
A standard figure of merit for quantifying the shielding capability of a material is the electromagnetic shielding effectiveness $SE$. It is defined as the ratio of the incident to the transmitted electric field amplitude, expressed on a logarithmic scale in decibels as in \cites{10518640}[p.~63]{Jaroszewski_Thomas_Rane_2019}
\begin{equation}
...
An electromagnetic wave can undergo reflection, absorption or multiple reflections within the shielding material, with each reflection contributing to the total reflected, absorbed, and transmitted wave. The total shielding effectiveness is determined by \cite[p. 63]{Jaroszewski_Thomas_Rane_2019}
...
Reflections contribute most significantly to a material’s shielding effectiveness \cite[p. 1]{Jaroszewski_Thomas_Rane_2019}. They are characterized by the reflection coefficient $R$. For a material to reflect incident fields, it must posses free charge carriers. For this reason, highly conductive materials, such as metals, reflect the majority of incident electromagnetic waves.

Reflections are caused by wave impedance mismatch between two materials. It is common practice to use the normalized wave impedance $Z$ to free-space wave impedance $Z_0$ when determining the reflection coefficient $R$. At the interface between free space and a shielding material, this yields \cite{Collin_2015}
...
The loss tangent is very low for the majority of materials. Ferrites are an exception, which are commonly used to dampen high frequency signals \cite{Balanis_2012}.



\subsubsection{Skin Effect}\label{sec:skin_effect}

The Skin effect describes the interaction of electromagnetic waves with conductors, causing eddy currents to form on the surface. To investigate this phenomenon, the complex wavenumber is introduced $\tilde{k}=k + i\kappa$, which is assists in mathematically describing waves propagating in lossy materials. The imaginary part $\kappa$ is expressed as
...
For highly conductive materials $\left(\sigma \ll \epsilon\omega\right)$ the skin depth is $d \propto 1/\sqrt{\omega}$. Therefore, high-frequency electromagnetic waves do not penetrate deeply into conductive materials, due to the free electrons associated with high conductivity absorbing the waves with eddy current on the material's surface and transforming the energy into heat. A material's permeability further reduces the skin depth due to the increased susceptibility to eddy currents \cite[p. 104]{Jaroszewski_Thomas_Rane_2019}.

\subsubsection{ASTM ES7-83 Method}\label{sec:astm}

The ASTM ES7-83 method is used to determine the shielding effectiveness of shielding materials in far-field condition. The shielding material is inserted into a coaxial TEM cell around the septum. Ideally, they form a continuous connection \cite{MORARI_BĂLAN_2015}. 
