...
\begin{equation}\label{eqn:const_current}
	{I}(z) = I_0.
\end{equation}
...
given in \eqref{eqn:const_current}, $I_0$ can be pulled out of the integral. Evaluating the remaining integral over the dipole 
...
The complex power $P_r$ is derived by integrating $\mathbf{S}$ over a closed surface around the dipole, which leads to \cite[p. 154]{Balanis_1997}

\begin{equation}
	P_r = \eta \frac{\pi}{3}  \left|\frac{I_0d}{\lambda}\right|^2 \left[ 1 - j \frac{1}{(kr)^3} \right].
...
Wires that are too long to be modeled as infinitesimal dipoles, but short enough to be considered electrically small ($\lambda / 50 < d \leq \lambda/10$), are classified as small physical dipoles \cite[pp. 162-163]{Balanis_1997}. These dipoles provide a more accurate and practical representation of linear wire antennas, and are examined in greater detail below.

A current $I_0$ is fed into the short, center-fed, linear antenna shown in \autoref{fig:linear_wire_antenna}. The current along the antenna arms $I(z)$ linearly drops to zero \cite[p. 412]{Jackson}, as visualized in \autoref{fig:electricdipolecurrent}. Mathematically, it is described by, 
...
satisfies $2\pi b < \lambda/10$~\cite[p.~231]{Balanis_1997}. The radiation 
...
The complex power density $\mathbf{S}$ can be derived following the same approach as for the electric dipole (see \eqref{eqn:com_pow_dens}). For the magnetic dipole, the imaginary part of $\mathbf{S}$ has the opposite sign compared to the electric dipole. This is the result of the near-field power being inductive in the case of the magnetic dipole, while it is capacitive for the electric dipole. The complex power equals \cite[p.~238]{Balanis_1997}
