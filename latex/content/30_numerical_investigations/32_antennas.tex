...
\subsection{Conceptual framework and modeling of antennas}
\todo[inline]{This section is TODO}
...
\end{equation}
 ...
 The RMS-value of the current through the feedpoint of the antennas are calculated with those S-parameters,
 
 \begin{equation}
 	I_{RMS,in} = \sqrt{P_{in}}\frac{(1-S_{AA})}{\sqrt{Z_0}},
 	\label{eqn:iin}
 \end{equation}
 ...
 \begin{equation}
 	V_{RMS,in} = \sqrt{P_{in}}(1-S_{AA})\sqrt{Z_0}.
 	\label{eqn:vin}
 \end{equation}
 
 Another method to derive voltages and currents is by integration of field intensities. Special care has to be taken at mesh refinement in the area of integration to reduce numerical errors. The impedance seen from the feedpoint is 
 
 \begin{equation}
 	Z_{in}=Z_0\frac{1+S_{AA}}{1-S_{AA}}.
 \end{equation}
 
 All values are peak values, unless otherwise stated. The septum of the TEM cell and the material of the antennas are perfectly conducting, to investigating results for coupling effects without distorting them with losses.
...
\todo{can plot quality be improved? Max delta S parameter reduction}
...
The current $I$ in the loop antenna changes along the antenna wire as shown in \autoref{fig:loopfeedreturncurrent}, indicating displacement current coupling back to the septum and back to the feedpoint, also seen in the slightly changing magnetic field intensity in \autoref{fig:loopnearfield}. Consequently, $\mathbf{m}_e$ gains a significant magnitude according to \autoref{eqn:me_i}, influencing the electric coupling behavior of the antenna. The feedpoint current is derived through integration of $\mathbf{H}$ in a closed loop of radius 0.11\,mm, measured 0.17\,mm above the feedpoint. The return path current is measured at the same height above the PEC surface. The results vary with position due of the displacement currents in the antenna's near-field.
...
An increase in gap height reduces the non-linearities in $\mathbf{m}_\mathrm{e}$ and $\mathbf{m}_\mathrm{m}$. The voltage drop across the gap and the charge accumulation remains stabler over frequency. A higher Q-factor leads to a lower cut-off frequency.
