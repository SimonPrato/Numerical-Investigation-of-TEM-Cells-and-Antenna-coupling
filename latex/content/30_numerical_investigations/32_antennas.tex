...
The coupling of various electrically small antennas placed in a TEM cell is investigated. Their electric and magnetic coupling is investigated through dipole moments. All antennas are fed with a power of 1\,W. Their material are made of a perfect electric conductor. Different positions and offsets are investigated and the results discussed. They are fed through a round wave port with a diameter of 0.46\,mm, and their wire has a diameter of 0.2\,mm and free-space permittivity. This results in a port impedance of $\approx 50$\,$\Omega$. The TEM cell used has a width of $a=40$\,mm, a height of $b=24$\,mm and a length of $l=100$\,mm, and its cells walls and septum consist of a perfect electric conductor, too. The TEM cell model does not contain the tapered sections at the ports. The ports have an impedance of $50\,\Omega$ due to the TEM cell's geometry.
...
When considering only the , only 
...
However, the dominant present mode is the TEM mode. Only the the electric dipole moment in y-direction and the magnetic dipole moment in x-direction are needed to model the antenna, because they align with the fields of the TEM mode.
...
The electric current flowing through the monopole antenna is aligned with the electric field $\mathbf{e}_n^\pm$ of the TEM mode. Therefore, according to \autoref{eqn:J1_propagating_waves}, this will lead to a coupling with the output ports.  \autoref{eqn:J1_propagating_waves} also requires $\mathbf{e}_n^\pm$ and the part of $I$ contributing to the output power to be in phase at the position of the monopole antenna, due to $\mathbf{e}_n^\pm$ and $\mathbf{h}_n^\pm$ being in-phase at the output ports. Integrating the current $I$ in \autoref{fig:monopolefeedcurrent} over the antenna length and multiplying it with $\mathbf{e}_\mathrm{TEM}^\pm$ yields

\begin{equation}
	\int_C \mathbf{e}^\pm_{\mathrm{TEM}}\boldsymbol{\tau} I(l)\,dl = 82.00\,\mu\mathrm{Am},
\end{equation}

which is equivalent to $\mathbf{m_e}=3.09\cdot 10^{-2}$, if normalized to the free-space wave impedance $Z_0\approx 376.73\,\Omega$.  
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{content/img/monopole_imp}
...
\autoref{fig:monopole_surface_currents} shows the surface current density on the septum caused by the monopole antenna. The current flows towards output ports in-phase, which causes the output power to be in-phase, too. This is a characteristic of the dominant electric dipole moment, predicted by .\todo{show for higher order modes, reference section where this is discussed} The current density is largest at the edge of the septum, where the TEM cell's wall is closest and charges accumulate. 
...
\autoref{monopole_surface_current_offset} shows the induced surface currents on the septum by the monopole antenna with an offset $x=7.5\,\mathrm{mm}$. 
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[b]{0.48\textwidth}
	\centering
\includegraphics[width=1\linewidth]{content/img/monopole_mag_energy}
...
\label{fig:monopoleelecenergy}
	...
	\caption{Peak magnetic and electric energy in the TEM cell generated by the monopole antenna}
...
A square loop antenna is placed in the TEM cell, with each side having a length of 1.6\,mm. This is preferable to a round antenna in numerical simulation, due to more accurate mesh modeling. \todo{Maybe describe this in the HFSS theory part? Modeling of round surfaces.} The antenna is aligned with the z-axis.
...
\label{fig:loopmagenergy}
	...
	\caption{Equivalent circuit of small loop antenna according to \cite[p. 244] {Balanis_1997}}
	\label{fig:example}
\end{figure}


The inductance consists of the antenna's inductance and the mutual inductance with the TEM cell. Similarly, the capacitance consists of the antenna's capacitance and the coupling capacitance with the TEM cell. 
%
...
\begin{tikzpicture}
	...
	\draw (6, 8) to[european resistor, l={$R$}] (6, 5);
	\draw (8, 8) to[european inductor, l={$L$}] (8, 5);
	\draw (10, 8) to[capacitor, l={$C$}] (10, 5);
	\draw (6, 8) -| (6, 9) -- (4, 9);
	\draw (6, 9) -- (8, 9) -| (8, 8);
	\draw (8, 9) -- (10, 9) -| (10, 8);
	\draw (10, 5) -| (10, 4) -- (4, 4);
	\draw (6, 5) -| (6, 4);
	\draw (8, 5) -| (8, 4);
	\draw[line width=0.7pt, -stealth] (4, 8) -- (4, 5);
	\node[shape=rectangle, minimum width=0.965cm, minimum height=0.715cm] at (3.75, 6.375){} node[anchor=north west, align=left, text width=0.577cm, inner sep=6pt] at (3.25, 6.75){$V$};
	\draw[line width=1.1pt, -latex] (6, 8) -- (6, 7.75);
	\node[shape=rectangle, minimum width=1.465cm, minimum height=0.965cm] at (6.75, 7.75){} node[anchor=north west, align=left, text width=1.077cm, inner sep=6pt] at (6, 8.25){$I_R$};
	\node[shape=rectangle, minimum width=1.465cm, minimum height=0.965cm] at (8.75, 7.75){} node[anchor=north west, align=left, text width=1.077cm, inner sep=6pt] at (8, 8.25){$I_L$};
	\draw[line width=1.1pt, -latex] (8, 8) -- (8, 7.75);
	\draw[line width=1.1pt, -latex] (10, 8) -- (10, 7.75);
	\node[shape=rectangle, minimum width=1.465cm, minimum height=0.965cm] at (10.75, 7.75){} node[anchor=north west, align=left, text width=1.077cm, inner sep=6pt] at (10, 8.25){$I_C$};
\end{tikzpicture}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{content/img/loop_opower}
	\caption{}
