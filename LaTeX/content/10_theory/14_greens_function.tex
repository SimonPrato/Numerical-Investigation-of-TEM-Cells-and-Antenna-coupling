...
Green's function describes the response of a linear differential operator $L$ to a point source as an input, which is modeled by delta-function $\delta$. Its general form is shown in \autoref{eqn:general_greens_funct}. 
...
Once \autoref{eqn:general_greens_funct} is solved and the Green's function $G$ of this specific operator is known, it can be used to solve any input function, such as $u(\mathbf{x})$ in \autoref{eqn:examplary_function_with_operator}. This is done by a convolution integral, shown in \autoref{eqn:examplary_function_solved}.
...
\subsubsection{Dyadic Green's Function}
