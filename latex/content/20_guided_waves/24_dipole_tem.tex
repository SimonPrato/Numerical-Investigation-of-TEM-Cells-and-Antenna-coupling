...
Now, the surfaces $S_1$ and $S_2$ are defined, which are depicted in \autoref{fig:displacement-current-tem-cell}. Surface $S_1$ starts from $S_0$ and runs parallel to the electric field $\mathbf{e}_n^\pm$, extending infinitely. A total surface is defined $S=S_0+S_1+S_2$, where $S_2$ closes the total surface around $S_1$ in infinity. 
...
Consequently, \cref{eqn:h_a_closed_int,eqn:h_b_closed_int} can be written with the newly defined closed surface $S$ as 
