...
Applying the integral form of the Lorentz reciprocity theorem in \autoref{eqn:lorentz_rec_theorem_int} with $\mathbf{J}_2=\mathbf{M}_1=\mathbf{M}_2=0$ yields
...
Using the modal expansions \cref{eqn:a_modal_superposition1,eqn:a_modal_superposition2}, \cref{eqn:b_modal_superposition1,eqn:b_modal_superposition2} for the fields $\mathbf{E}$ and $\mathbf{H}$ radiated by $\mathbf{J}_1$ lead to
...
Due to the orthogonal property of \autoref{eqn:norm_power} and the normalization in \autoref{eqn:unit_power}, the coefficients of each mode can be evaluated separately with
...
Assuming only the TEM mode can propagate, combining \autoref{eqn:unit_power} with the fields $\mathbf{e}_\mathrm{TEM}$ and $\mathbf{h}_\mathrm{TEM}$ with their respective coefficients $a_\mathrm{TEM}$, $b_\mathrm{TEM}$ leads to \cite{4091811}

\begin{subequations}
	\begin{equation}
		P_{\mathrm{out1}}=\iint_S \langle \mathbf{S} \rangle \cdot \mathrm{d}\mathbf{s'}= \iint_S \frac{1}{2} \, \Re \{ \left(a\cdot \mathbf{e}_\mathrm{TEM}^\pm\right) \times \left(a\cdot \mathbf{h}_\mathrm{TEM}^\pm\right)^* \}\cdot \mathrm{d}\mathbf{s'} = \frac{|a_\mathrm{TEM}|^2}{2},
		\label{eqn:power_of_poynting1}
	\end{equation}
	\begin{equation}
		P_{\mathrm{out2}}=\iint_S \langle \mathbf{S} \rangle \cdot \mathrm{d}\mathbf{s'}= \iint_S \frac{1}{2} \, \Re \{ \left(b\cdot \mathbf{e}_\mathrm{TEM}^\pm\right) \times \left(b\cdot \mathbf{h}_\mathrm{TEM}^\pm\right)^* \}\cdot \mathrm{d}\mathbf{s'} = \frac{|b_\mathrm{TEM}|^2}{2}.
...
This formulation assumes, that the magnetic field strength $\mathbf{h}^\pm$ does not change over the loop area. This is the case for electrically small loops. Otherwise, the magnetic field strength $\mathbf{h}^\pm$ must be considered in the integration process of \autoref{eqn:mag_dipole_moment_tem} \cite{Collin_2015}.

If several modes are propagating, it is useful to determine the coefficients $a_n$ and $b_n$ weighting the modes in \crefrange{eqn:a_modal_superposition1}{eqn:b_modal_superposition2}. In this case, the orthogonality property in \autoref{eqn:norm_power} can be used to derive  \cite{Collin_2015}
...
For the TEM mode, the relation between $\mathbf{e}^\pm_\mathrm{TEM}$ and $\mathbf{h}^\pm_\mathrm{TEM}$ expressed in \autoref{eqn:tem_h_e_relation} yields to a simplified form of \autoref{eqn:a_b_moments}, written as \cite{Sreenivasiah_Chang_Ma_1981}

\begin{equation}
    \begin{pmatrix}a_\mathrm{TEM} \\b_\mathrm{TEM}\end{pmatrix} =-\frac{1}{2}(\mathbf{m_e}\pm jk\mathbf{m_m}\times \mathbf{z})\cdot \mathbf{e}_\mathrm{TEM}^\pm.
    \label{eqn:a_b_moments_simp}
\end{equation}

\autoref{eqn:a_b_moments_simp} is useful in investigations further in this thesis, because it requires knowledge of only $\mathbf{e}^\pm_\mathrm{TEM}$ to determined the dipole moments. The dipole moments $\mathbf{m}_{\mathrm{e}}$ and $\mathbf{m}_{\mathrm{e}}$ are separately derived by 

\begin{subequations}
\begin{equation}
	\mathbf{m}_{\mathrm{e}}=\frac{a_\mathrm{TEM}+b_\mathrm{TEM}}{\mathbf{e}_\mathrm{TEM}^\pm},
	\label{eqn:ifa_me}
\end{equation}

\begin{equation}
	\mathbf{m}_{\mathrm{m}}=j\frac{a_\mathrm{TEM}-b_\mathrm{TEM}}{k_0  \mathbf{e}_\mathrm{TEM}^\pm}.	\label{eqn:ifa_mm}
\end{equation}
\end{subequations}

The individual components of the electric field for a given mode $\mathbf{e}^\pm_n=e_n,x^\pm\cdot \mathbf{\hat{a}}_x+e_n,y^\pm\cdot \mathbf{\hat{a}}_y+e_n,z^\pm\cdot \mathbf{\hat{a}}_z$ can be analyzed separately by examining the output power of the TEM cell. For example, the components of $\mathbf{m}_\mathrm{e}$ are derived with
...
For the next higher-order mode TE\textsubscript{01}, the situation is reversed. An electric dipole moment leads to a phase shift of $\pm\pi$, while a magnetic dipole moment produces no phase shift. This behavior is again due to the phase shift occurring now between the electric field intensities at the output ports, $\mathbf{e}_\mathrm{TE01}^+$ and $\mathbf{e}_\mathrm{TE01}^-$, while there is no phase shift between the magnetic field intensities, $\mathbf{h}_\mathrm{TE01}^+$ and $\mathbf{h}_\mathrm{TE01}^-$.
...
The induced voltage $V_n^+$ is related to the fields $\mathbf{e}_n^+$ and $\mathbf{h}_n^+$ at the output port located in positive $z$-direction, while the induced voltage $V_n^-$ to the fields at the other output port $\mathbf{e}_n^-$ and $\mathbf{h}_n^-$. For the TEM mode, the total induced voltage equals $V_\mathrm{TEM}=V_\mathrm{TEM}^--V_\mathrm{TEM}^+$. The relation to the magnetic dipole moment $\mathbf{m}_m$ is expressed as

\begin{equation}
	\mathbf{m}_m=\frac{a_\mathrm{TEM}-b_\mathrm{TEM}}{\mathbf{e}_\mathrm{TEM}^\pm\cdot k_0}=\frac{V_\mathrm{TEM}}{\mathbf{e}_\mathrm{TEM}^\pm\cdot k_0}.
	\label{eqn:m_v}
\end{equation}

For the TE\textsubscript{01} mode, the total induced voltage equals $V_\mathrm{TE01}=V_\mathrm{TEM}^-+V_\mathrm{TE01}^+$. 

In the general case, a magnetic dipole moment $\mathbf{m}_m$ producing fields characterized with coefficients $a_n$ and $b_n$ models the magnetic coupling behavior of any electrically small antenna yielding the same coefficients. Consequently, deriving an equivalent magnetic dipole moments $\mathbf{m}_m$ of an electrically small antenna is possible by measuring $a_n$ and $b_n$ at the output ports. 

In a similar manner to \cref{eqn:e_int_a,eqn:e_int_b}, a constant magnetic field $\mathbf{h}_n^\pm$ along a magnetic current $I_m$ following a curve $C$, makes following expression possible:
...
Now, the surfaces $S_1$ and $S_2$ are defined. Surface $S_1$ starts from $S_0$ and runs parallel to the electric field $\mathbf{e}_n^\pm$, extending infinitely. A total surface is defined $S=S_0+S_1+S_2$, where $S_2$ closes the total surface around $S_1$ in infinity. Therefore, \cref{eqn:h_a_closed_int,eqn:h_b_closed_int} can be written with the newly defined closed surface $S$ as 

\todo[inline]{TODO: Hier wäre vielleicht eine Skizze mit $S_1$, $S_2$ angebracht.}
...
For the TEM mode, relating the obtained \cref{eqn:charges_a,eqn:charges_b} to the electric dipole moment from \autoref{eqn:ifa_me} with a total current $I_\mathrm{TEM}=I_\mathrm{TEM}^+ + I_\mathrm{TEM}^-$ delivers

\begin{equation}
	\mathbf{m}_e=\frac{a_\mathrm{TEM}+b_\mathrm{TEM}}{\mathbf{e}_\mathrm{TEM}^\pm}=\frac{I_\mathrm{TEM}}{\mathbf{e}_\mathrm{TEM}^\pm}.
	\label{eqn:me_i}
\end{equation}

An electric dipole moment $\mathbf{m}_e$ producing fields characterized with coefficients $a_n$ and $b_n$ models the electric coupling behavior of any electrically small antenna yielding the same coefficients. Consequently, deriving an equivalent $\mathbf{m}_e$ of an electrically small antenna is possible by measuring $a_n$ and $b_n$ at the output port. 
