\section{Numerical Shielding Investigations in TEM Cells}\label{sec:shielding-sim}

The theoretical shielding framework developed in \cref{sec:shielding} is applied 
numerically to three materials, namely silver, ferrite, and barium titanate. 
These materials possess high conductivity, lossy permeability, and lossy 
permittivity, respectively, in order to assess the effect of each characteristic 
on the overall shielding performance. The shielding effectiveness is first 
evaluated in the far-field using the ASTM ES7-83 method, followed by separate 
determination of the electric and magnetic shielding effectiveness in the near-field 
using the dual TEM cell approach. The results are then applied to shielding 
enclosures around the monopole and loop antennas, demonstrating the consistency 
between the near-field character of each antenna and the shielding performance of 
the respective materials. Finally, the full antenna models are replaced by their 
equivalent dipole moments to assess the accuracy of this reduced representation 
...
performance can be investigated separately. Ferrite is additionally modeled with a 
magnetic loss tangent of $\tan \delta_m = 0.05$ and barium titanate with an electric 
loss tangent of $\tan \delta = 0.0095$.

The shielding material is modeled as a thin sheet of thickness $10\,$µm, positioned 
at the center of the TEM cell at $z = 0$. Since accurately resolving such a thin 
sheet within a large computational domain would require a large number 
of mesh elements, the sheet is represented using impedance boundary conditions 
\cite{1430946}. In this approach, the shielding material is removed from the 
computational domain and replaced by boundary conditions imposed on its surface, rather than being discretized as a solid body within 
the computational domain.
Plane wave propagation normal to the surface is assumed for this method, which is a 
good approximation here since the sheet surfaces are oriented perpendicular to the 
wave propagating within the TEM cell.

The ASTM ES7-83 method requires the definition of a reference power 
$P_\mathrm{ref}=1\,\mathrm{W}$ to evaluate the shielding effectiveness according 
to \eqref{eqn:SE_power}. The transferred power $P_\mathrm{load}$ between the TEM 
cell ports in the presence of the shielding material is then determined numerically, 
from which the shielding effectiveness as a function of frequency is computed for 
each material. The resulting shielding effectiveness is presented against frequency 
in \autoref{fig:se-barrium-ferrite}.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1\linewidth]{content/img/se-barrium-ferrite}
	\caption{Shielding effectiveness as a function of frequency for 10\,µm thick sheets of ferrite, barium titanate, and silver, determined using the ASTM ES7-83 method.}
...
Silver, as a highly conductive material, exhibits a large reflection loss $R_\mathrm{dB}$ owing to the significant impedance mismatch between the material and the surrounding air. This mismatch decreases with increasing frequency, as the wave impedance of silver rises with frequency in accordance with \eqref{eqn:rel_wave_imp}. Simultaneously, the absorption coefficient $A_\mathrm{dB}$ increases with frequency, consistent with the reduction in skin depth at higher frequencies, as shown in detail in \cref{sec:irt_waves}. At low frequencies, the multiple-reflection correction term $B_\mathrm{dB}$ is negative and partially offsets the reflection loss, owing to the destructive 
interference of internal reflections with the primary reflected wave, as 
discussed in \cref{sec:irt_waves}. As frequency increases, however, the sheet becomes electrically thicker, rendering $B_\mathrm{dB}$ negligible while the 
...
coefficient $A_\mathrm{dB}$ is limited by its low bulk conductivity $\sigma$ and low magnetic 
...
frequency of approximately $3\,\mathrm{GHz}$. 

To achieve accurate results, sufficient mesh resolution in the aperture region 
is critical. Since the aperture is significantly smaller than the computational 
domain, inadequate resolution introduces large numerical inaccuracies, as 
highlighted in \cref{sec:adap_sol_pro}. Following the procedure of the dual TEM cell method outlined in \cref{sec:dual_tem_cell}, the 
...
\begin{figure}[htbp]
	...
	\caption{The sum $P_\mathrm{ref,sum}$ and difference $P_\mathrm{ref,diff}$ of the 
		reference power, computed with an empty aperture.}
...
\begin{figure}[htbp]
	...
	\caption{The sum $P_\mathrm{load,sum}$ and difference $P_\mathrm{load,diff}$ of 
		the output power measured at ports 3 and 4 with (a) silver, (b) ferrite and barium titanate filling the aperture.}
	\label{fig:powerdualtem}
\end{figure}
...
leads to a strong impedance mismatch with electric near-field sources. The magnetic shielding 
effectiveness increases monotonically across the inspected frequency range, driven by 
the growing absorption contribution as the skin depth decreases substantially as $d\propto 1/\sqrt{\omega}$ (see 
\eqref{eqn:kappa}). The electric shielding effectiveness, by contrast, begins to 
decline at higher frequencies, as the wave impedance of the material increases 
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\raisebox{1.5mm}{\includegraphics[width=1\linewidth]{content/img/monopole_antenna_enclosure}}
		\caption{Monopole antenna}
		\label{fig:monopoleantennaenclosure}
	\end{subfigure}
	\caption{Investigated (a) loop and (b) monopole antennas placed inside a hollow cubic enclosure within the 
		TEM cell.}
	\label{fig:antennaenclosure}
\end{figure}

The radiated power from the antennas with and without a barium titanate enclosure is shown in \autoref{fig:bariumenclosurepower}. The output power of the loop antenna increases 
...
The corresponding radiated power for the ferrite enclosure is shown in \autoref{fig:ferriteenclosurepower}. In this case, the output power 
of the monopole antenna increases due to the ferrite enclosure inductively loading the 
antenna, while the loop antenna's radiation is attenuated. As discussed in detail in \cref{sec:irt_waves}, ferrite proves to be an 
effective magnetic shield in this context, owing to its high permeability, 
which redirects the magnetic flux away from the shielded region.
...
\begin{figure}[htbp]
	...
	\caption{Radiated power as a function of frequency for the loop and monopole 
		antennas with and without (a) barium titanate and (b) ferrite enclosure. }
	\label{fig:enclosurepower}
\end{figure}

The influence of enclosure wall thickness on the radiated power is examined in 
\autoref{fig:enclosurepowervarthick}, where silver is additionally included as a shielding material. As expected, increasing the wall thickness reinforces the 
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	\caption{Radiated power as a function of frequency for the (a) loop and (b) monopole 
		antennas with varying enclosure wall thickness.}
	\label{fig:enclosurepowervarthick}
\end{figure}

Additionally, resonances are observed for thick enclosures, manifesting as a pronounced peak 
in the transmitted power at approximately $2.5\,\text{GHz}$ for the barium titanate enclosure 
with a wall thickness of $100\,\upmu\text{m}$. This behavior is attributed to a resonance 
between the loop antenna and the barium titanate enclosure, which is evidenced by the 
pronounced dip in impedance magnitude and the corresponding phase shift at approximately 
$2.5\,\text{GHz}$ visible in \autoref{fig:shieldloopimp} and derived with the S-parameters by \eqref{eqn:za}. A corresponding effect is 
anticipated for the ferrite enclosure of thickness 100\,µm. The associated resonance frequency, however, 
lies just outside the investigated frequency range.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1\linewidth]{content/img/shield_loop_imp}
	\caption{Impedance magnitude and phase of the loop antenna enclosed in the barium titanate 
		shielding enclosure with a wall thickness of $100\,\upmu\text{m}$ as a function of frequency.}
	\label{fig:shieldloopimp}
\end{figure}


%As shown in 
%\autoref{fig:loop-resonance}, this effect corresponds to a half-wavelength resonance 
%established within the enclosure cavity. The excitation of such resonant waves requires, among other things, that reflections on the material boundary are not fully suppressed by the
%multiple-reflection correction term $B_\mathrm{dB}$, as discussed in \cref{sec:irt_waves}. A larger wall thickness supports this condition by increasing the electrical 
...
enclosures enables investigation of shielding performance with reduced 
...
in barium titanate and for the monopole antenna in ferrite. As the enclosure material becomes a more effective shield against the dominant near-field of the respective antenna, however, the accuracy of the approximation decreases, indicating that the equivalent dipole 
moments must be adjusted accordingly to account for the interaction between the source 
and the enclosure.

The discrepancy in radiated power can be explained by examining the eddy currents induced 
on the shielding enclosure, shown in \autoref{fig:eddy}. Although both sources induce 
comparable eddy current magnitudes, the eddy currents induced by the loop antenna return 
to the feedpoint, exhibiting an overall different current pattern. The shielding material thus interacts 
with the source and modifies its radiation characteristics. In the dipole representation, 
however, the moments act as impressed sources, and the associated currents are purely 
rotational. Consequently, the interaction 
between the source and the shielding material is neglected in the dipole model, very likely leading to the large difference in output power.

\begin{figure}[htbp]
	\centering
	\begin{subfigure}[b]{0.5\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{content/img/eddy_loop}
	...
	\begin{subfigure}[b]{0.46\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{content/img/eddy_dipoles}
		\caption{Loop antenna}
		\label{fig:eddydipoles}
	\end{subfigure}
	\caption{Eddy currents induced on the silver shielding enclosure of thickness 
		10\,\textmu m for the dipole moments in (a) and for the loop antenna in (b), 
		depicted on the surface of the shielding enclosure behind the source.}
	\label{fig:eddy}
\end{figure}
