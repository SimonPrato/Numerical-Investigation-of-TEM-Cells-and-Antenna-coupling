...
The magnetic dipole moment characterizes the strength of a magnetic source. An 
electrically small current loop carrying a current $I_0$ serves as the 
model for a magnetic dipole, as shown in \autoref{fig:magnetic_dipole_drawing}. 
This approximation is valid provided the loop circumference satisfies 
$2\pi b \ll \lambda/10$~\cite[p.~231]{Balanis_1997}. The radiation pattern of the magnetic 
dipole is equal to that of the electric dipole, with the roles of the electric and 
magnetic fields interchanged~\cite[p.~254]{Griffiths_2024}.

The magnetic dipole moment $\mathbf{M}_m$ has units of $\mathrm{A\,m^2}$ and is 
defined for a current loop as~\cite[p.~361]{Collin_2015}
...
where $\mathbf{A} = A\,\hat{\mathbf{a}}_z$ is the vector area of the loop, with 
magnitude $A$ and unit normal aligned along the $z$-direction.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\linewidth]{content/img/magnetic_dipole_drawing.png}
	\caption{Current loop of radius $b$ carrying a current $I_0$ that generates a 
		magnetic dipole moment $\mathbf{M}_m$. Equivalently, the same dipole moment can 
		be produced by a magnetic current $I_\mathrm{m}$ flowing perpendicular to the 
		plane of the loop over a distance $L$.}
	\label{fig:magnetic_dipole_drawing}
\end{figure}

An equivalent representation replaces the current loop with a magnetic current $I_m$ 
flowing over a distance $\mathbf{L} = L\,\hat{\mathbf{a}}_z$, with
magnitude $L$ and unit normal aligned along the $z$-direction. This yields an alternative definition of the magnetic dipole 
moment with units of $\mathrm{V\,m}$~\cite{Kreindl_Bauernfeind_Weiss_Stockreiter_Kaltenbacher_2024},
...
This definition is widely adopted in the literature and in numerical simulation codes~\cite{Kreindl_Bauernfeind_Weiss_Stockreiter_Kaltenbacher_2024}. 
Since both definitions describe the same physical source, a relation exists, expressed as~\cites[p.~237]{Balanis_1997}{Kreindl_Bauernfeind_Weiss_Stockreiter_Kaltenbacher_2024}
...
so that the infinitesimal magnetic current element and the electrically small 
current loop are equivalent representations producing identical electric and 
magnetic field distributions. The electric field intensity $\mathbf{E}$ of the magnetic dipole is given by
