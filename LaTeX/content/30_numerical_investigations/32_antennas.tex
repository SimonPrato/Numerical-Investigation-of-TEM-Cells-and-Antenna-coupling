...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.3\linewidth]{content/img/antenna_port}
	\caption{Geometry of an antenna's feedpoint. The inner conductor is a PEC wire of the antenna. The outer conductor is the TEM cells PEC confinement. In between, vacuum with free-space permittivity $\epsilon_0$ is located.}
	\label{fig:antennaport}
\end{figure}


In this sections, different antennas are placed in the TEM cell. Their electric and magnetic coupling is investigated through dipole moments. All antennas are fed with a power of 1\,W. Their material are made of a perfect electric conductor. Different positions and offsets are investigated and the results discussed. They are fed through a round wave port with a diameter of 0.46\,mm, and their wire has a diameter of 0.2\,mm and free-space permittivity. This results in a port impedance of $\approx 50$\,$\Omega$. The TEM cell used has a width of $a=40$\,mm, a height of $b=24$\,mm and a length of $l=100$\,mm, and its cells walls and septum consist of a perfect electric conductor, too. The TEM cell model does not contain the tapered sections at the ports. The ports have an impedance of $50\,\Omega$ due to the TEM cell's geometry.
...
\begin{equation}
	\iint_S \mathbf{e_0} \times \mathbf{h_0} \cdot d\mathbf{s}' = 1,
	\label{eqn:normalization of fields}
\end{equation}

where the surface $S$ spans over one of the output ports. The fields are linearly scaled by the complex coefficients $a$ and $b$. Only the TEM mode is considered, therefore only one such pair is needed. \todo{Write preparations. Near field plots missing.}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{content/img/monopole_output_power}
...
\begin{figure}[htbp]
	...
	\centering
\centering
\includegraphics[width=0.9\linewidth]{content/img/monopole_current_dist}
\caption{Current distribution along monopole antenna arm}
...
\label{fig:loopmagenergy}
	...
	\caption{Electric and magnetic energy in the TEM cell, produced by the loop antenna.}
	\label{fig:example}
\end{figure}

The total energy density is given by \cite[p.]{Griffiths_2024}

\begin{equation}
	W = \frac{1}{2}\left(\underbrace{\epsilon E^2}_{\text{Electric energy}} + \underbrace{\frac{1}{\mu} B ^ 2}_{\text{Magnetic energy}}\right)
\end{equation}
...
\todo{Electrically short antennas have to be either inductive or capactive. Therefore, they all behave as monopole and current loop antenna. Find sources. Two questions arise: Can an antenna be built, which has a dominant electric dipole moment AND some significant magnetic dipole moment? And how does the antenna impedance relate to the moments in case of 90° turned center-fed monopole antenna?}
